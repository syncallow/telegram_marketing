...
const form = useForm({
    ...
    enable_to_send: props.post.enable_to_send,
    frequency: props.post.frequency,
    chat_ids: props.post.chats
})
...
<template>
    ...
    <MainLayout>
        ...
        <form class="max-w-lg mx-auto">
            ...
            <div class="mb-3">
                <label for="frequency" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Частота рассылки</label>
                <select v-model="form.frequency" id="frequency" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-emerald-500 dark:focus:border-emerald-500">
                    <option>Выберите частоту</option>
                    <option :selected="form.frequency === 'daily'" value="daily">Каждый день</option>
                    <option :selected="form.frequency === 'weekly'" value="weekly">Каждую неделю</option>
                    <option :selected="form.frequency === 'monthly'" value="monthly">Каждый месяц</option>
                </select>
            </div>
            <div class="mb-3">
                <label for="chats" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Чаты для рассылки</label>
